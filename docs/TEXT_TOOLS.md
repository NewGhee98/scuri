...
Implemented locally on `feat/page-text`, 26 September 2026. **Not published or deployed.** [START_HERE](../START_HERE.md) owns release status; [verification evidence](evidence/2026-09-26-text-tools.md) records the tested scope.
...
Reusable template rows have dedicated columns for frames and background and no general template snapshot. They need one additive JSON field to sync text across devices. The review-only migration [20260925170000_add_template_text_layers.sql](../supabase/migrations/20260925170000_add_template_text_layers.sql) adds `templates.text_layers jsonb NOT NULL DEFAULT '[]'` with an array constraint. **It has not been run.** It does not alter RLS/grants, project rows, assets or originals. Inspect the target schema and approve the normal migration process before releasing reusable template text; `IF NOT EXISTS` is not verification that an existing column has the intended constraint/default.
