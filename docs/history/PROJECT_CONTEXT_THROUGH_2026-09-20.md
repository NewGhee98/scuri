# Historical Scuri context and release decisions — through 20 September 2026

**Historical archive, captured 20 September 2026.** The sections below preserve decisions and dated observations through the PR #29 release, including release notes that previously existed only in the working tree. Statements such as “current”, “not deployed”, “local only” and “resume first” describe their original stage; they are not present-day status or instructions. Later production sections record subsequent approvals and releases. The original four Drive correction banners were later replaced by consistent summaries in the documentation handoff task.

Start with [START_HERE](../../START_HERE.md) for current status and [PROJECT_CONTEXT](../../PROJECT_CONTEXT.md) for implementation. Prior configuration claims are historical observations, not a fresh service audit. Original machine-only evidence pointers have been replaced with names and the [portable evidence index](../evidence/README.md); missing source artifacts are disclosed there. Do not rerun old patch, migration, deployment or recovery instructions.

---

_Last updated: 2026-09-20_

## Historical record — How to use this file
...
## Historical record — Handoff between sessions / agents
...
## Historical record — Product summary
...
## Historical record — Ironclad product requirement: templates must persist across devices
...
## Historical record — Ironclad product requirement: projects must persist across devices
...
## Historical record — Navigation / project UX decisions
...
## Historical record — Templates feature
...
### Historical record — Curated editorial starter pack
...
### Historical record — Template library filters
...
### Historical record — Image repositioning
...
## Historical record — GitHub / release workflow
...
## Historical record — Safari / iPhone issue already addressed
...
### Historical record — Correct browser environment variables
...
### Historical record — Mis-prefixed integration variables
...
## Historical record — Supabase database state verified during the 2026-08-12 release
...
## Historical record — Cross-device acceptance test
...
## Historical record — Production vs Preview rule
...
## Historical record — Release status — 2026-08-12
...
## Historical record — Release status — 2026-08-31 (not yet merged)
...
## Historical record — Release status — 2026-09-12
...
## Historical record — Local safety fix — 2026-09-13 (not deployed)
...
## Historical record — Product backlog / later ideas
...
## Historical record — Approved client safety and backup release — 2026-09-13 (not deployed)
...
## Historical record — Zoom and arrangement suggestions — 2026-09-13 (not deployed)
...
## Historical record — Upload-checkpoint protection recovery — 2026-09-13
...
## Historical record — Four built-in panorama choices — 2026-09-15 (local, not deployed)
...
## Historical record — Panorama production release — 2026-09-15
...
This release supersedes the panorama section's pre-deployment status above. The new production result is recorded in the local handoff and `SCURI_PANORAMA_PRODUCTION_2026-09-15.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)); the committed implementation documents describe verification before deployment. The photo safety invariant, separate Islands recovery limitation and previously documented sync limitations are unchanged. Live photo upload/export, cross-device persistence and physical-device gestures were not exercised by this deployment check.

## Historical record — Editor alignment, preview and library cleanup — 2026-09-16
...
### Historical record — Preservation and compatibility invariant
...
## Historical record — Editor production release — 2026-09-16
...
This verified release supersedes the implementation section's pre-deployment status. The post-deployment record is local only, with full evidence in `SCURI_EDITOR_PRODUCTION_2026-09-16.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)); the deployed commit retains its pre-deployment implementation/test record. Photo safety, storage authority, explicit cleanup and separate Islands recovery limitations remain as documented above.

## Historical record — Add-page replacement diagnosis — 2026-09-17
...
## Historical record — Add-page intent fix — 2026-09-17 (deployed and verified)
...
## Historical record — Add-page production release — 2026-09-17
...
This post-deployment record is local only; the deployed commit contains the implementation and validation record prepared before deployment. Full evidence is in `SCURI_ADD_PAGE_PRODUCTION_2026-09-17.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)) and `SCURI_ADD_PAGE_PUBLIC_RELEASE_2026-09-17.json`.

## Historical record — Session photo previews and composition guides — 2026-09-18 (deployed and verified)
...
This post-deployment record is local only; the deployed commit contains the implementation and validation record prepared before deployment. Full evidence is in `SCURI_SESSION_PREVIEWS_PRODUCTION_2026-09-18.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)) and `SCURI_SESSION_PREVIEWS_PUBLIC_RELEASE_2026-09-18.json`. Existing Islands recovery limitations remain unchanged.

## Historical record — Export quality review and proportional output — 2026-09-18 (local implementation)
...
No migration, live project edit, asset modification, Drive-original change, credential/configuration change or storage rewrite was performed during release. The photo safety invariant and Islands recovery limitation remain unchanged. This post-deployment record is local only; the deployed documentation contains the pre-deployment implementation/validation record. Evidence: `SCURI_EXPORT_QUALITY_PRODUCTION_2026-09-18.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)) plus the branch, main and public verification JSON files.

## Historical record — Project photos rebuild — 2026-09-19 (implemented locally, not deployed)
...
## Historical record — Project photos iPad Preview — 19 September 2026
...
Preview and Production share the configured Supabase URL/publishable-key variables. Signing in to Scuri can expose the same real projects; use a fresh disposable test project. Direct Drive import's Picker key/app ID are absent from Vercel; Google Photos API/consent and the exact preview OAuth origin remain unverified. Files/Photos are the initial iPad acceptance path. Do not claim live OAuth or physical-device acceptance from this release. Evidence: `library-preview-branch-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `library-preview-http-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), and `SCURI_LIBRARY_PREVIEW_2026-09-19.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)).

## Historical record — Project photos production — 19 September 2026
...
Evidence: `library-production-main-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `library-production-public-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), and `SCURI_LIBRARY_PRODUCTION_2026-09-19.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)). This post-release record is local only; deployed documentation retains the implementation and acceptance checklist.

## Historical record — Photo viewing space — 19 September 2026 (implemented locally, not deployed)
...
The image stage occupies 85.1% of a 1180x820 viewport and 89.7% of an 820x1180 viewport with controls visible; hiding controls uses the full stage. These are browser-layout measurements, not physical iPad certification. Visual review covered the library, landscape photo, panorama and phone selector. Evidence: `scuri-viewing-space-final/browser-report.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), screenshots and JPEG; overview: `SCURI_PHOTO_VIEWING_SPACE_2026-09-19.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)).
...
## Historical record — Photo viewing space production — 19 September 2026
...
Evidence: `viewing-release-branch-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `viewing-production-main-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `viewing-production-public-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), and `SCURI_PHOTO_VIEWING_PRODUCTION_2026-09-19.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)). This post-release record is local only; deployed documentation contains the implementation and validation record above.

## Historical record — Add-page template filters — 19 September 2026 (implemented locally, not deployed)
...
No live project, Supabase record, Drive file or production deployment was modified. The safety invariant remains: missing bytes never mean deletion; existing assignments and independent crops remain authoritative. Historical Islands recovery limitations are unchanged. Evidence: `SCURI_ADD_PAGE_FILTERS_2026-09-19.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `scuri-template-filters-tests.txt` and `scuri-template-filters-build.txt`.

## Historical record — Add-page template filters production — 20 September 2026
...
Evidence: `template-filters-release-branch-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `template-filters-production-main-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `template-filters-production-public-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), and `SCURI_ADD_PAGE_FILTERS_PRODUCTION_2026-09-20.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)). This post-release record is local only; deployed documentation contains the implementation and validation record above.

## Historical record — Subtle selection shading — 20 September 2026 (implemented locally, not deployed)
...
Physical iPad Air 11-inch M2 touch targeting, pinch/drag, Pencil and performance remain untested; browser viewport and synthetic touch-event checks do not certify hardware behaviour. The existing handle sizes are unchanged. No live project, Supabase record, Drive file or deployment was modified. Missing local bytes still never imply deletion, and the historical Islands recovery limitation is unchanged. Evidence: `SCURI_SELECTION_SHADING_2026-09-20.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `scuri-selection-tests.txt` and `scuri-selection-build.txt`.

## Historical record — Subtle selection shading production — 20 September 2026
...
Evidence: `selection-release-branch-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `selection-production-main-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `selection-production-public-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), and `SCURI_SELECTION_PRODUCTION_2026-09-20.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)). This post-release record is local only; deployed documentation contains the implementation and validation record above.

## Historical record — Canvas viewport navigation — 20 September 2026 (implemented locally, not deployed)
...
Evidence: `scuri-viewport-tests.txt` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `scuri-viewport-lint.txt`, `scuri-viewport-build.txt` and `SCURI_CANVAS_VIEWPORT_2026-09-20.md`.

## Historical record — Canvas viewport production — 20 September 2026
...
Evidence: `viewport-release-branch-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `viewport-production-main-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `viewport-production-public-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), and `SCURI_CANVAS_VIEWPORT_PRODUCTION_2026-09-20.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)). This post-release record is local only; deployed documentation contains the implementation and validation record above.

## Historical record — Free photo positioning — 20 September 2026 (implemented locally, not deployed)
...
**Testing limitation:** desktop Chrome and automated pointer/geometry checks do not certify physical iPad Air 11-inch M2 Safari pinch, Pencil, touch targeting, orientation or performance. No physical-device test or live cloud round trip was performed during implementation. Evidence: `SCURI_FREE_POSITIONING_2026-09-20.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)) and `scuri-free-position-{tests,typecheck,lint,build}.txt`.

## Historical record — Free photo positioning production — 20 September 2026
...
Evidence: `free-position-release-baseline.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `free-position-release-branch-verification.json`, `free-position-production-main-verification.json`, `free-position-production-public-verification.json`, and `SCURI_FREE_POSITIONING_PRODUCTION_2026-09-20.md`. This post-release record is local only; deployed documentation contains the implementation and validation record above.

## Historical record — Template design tools — Change 5 (local implementation, not deployed)
...
Physical iPad Air 11-inch M2 Safari touch, Pencil, pinch, virtual keyboard and performance remain untested. Desktop viewport/hit-target checks and synthetic pointer callbacks do not certify physical-device behaviour. Live cloud synchronization was not exercised; persistence coverage uses the existing transport with a fake client. No live project, asset, original, migration or deployment was changed. Evidence: `SCURI_TEMPLATE_DESIGN_2026-09-20.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `scuri-template-layout-tests.txt`, `scuri-template-layout-typecheck.txt`, `scuri-template-layout-lint.txt` and `scuri-template-layout-build.txt`.

## Historical record — Template design tools production — 20 September 2026
...
Evidence: `template-tools-release-baseline.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `template-tools-release-branch-verification.json`, `template-tools-production-main-verification.json`, `template-tools-production-public-verification.json`, and `SCURI_TEMPLATE_TOOLS_PRODUCTION_2026-09-20.md`. This post-release record is local only; deployed documentation contains the implementation and validation record above.

## Historical record — Project photos scrolling and documentation — 20 September 2026 (tested, awaiting release)
...
**Documentation reconciliation:** README, this handoff, VERIFICATION and the photo-library acceptance checklist now describe current features/status. Historical sync/client/zoom/checkpoint notes are explicitly labelled; migration-review and Islands-recovery notes distinguish the user's reported schema confirmation from independent verification and retain the no-repair/no-repeat-migration boundary. The stale missing `CLAUDE_START_HERE.md` reference was replaced with the current handoff/checklist. All four documents in the signed-in Drive `App Projects / scuri` folder were read, updated and exported for read-back: Product Overview, Status & Roadmap, Upcoming Features and Change Timeline. Current-state documents no longer describe project metadata as device-only or positioning/backups as unbuilt. Every original timeline text line survives, with older handoff headings dated as historical; the September release history and pending scrolling fix are added. They clearly distinguish deployed PR #28 from this fix awaiting release. All four read-backs contain main `8398085` and current 512-test verification. Before/after evidence is in `scuri-docs-2026-09-20/` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)).

No live project/asset/original, Supabase record, migration or credential was modified. Drive writes were restricted to the four requested documentation files, retaining their private sharing. Supabase remains authoritative for metadata and assignments; Drive stores image bytes; missing bytes never imply deletion. This work does not recover historical Islands mappings. Evidence: `SCURI_SCROLL_AND_DOCUMENTATION_2026-09-20.md` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)) and `scuri-scroll-{before-tests,tests,typecheck,lint,build}.txt`.

### Historical record — Scrolling fix publication — PR #29, Preview ready

[PR #29](https://github.com/NewGhee98/scuri/pull/29), "Fix Project photos scrolling and refresh Scuri documentation", is open against main. Published head `f777149c616029462792d8d419ba5f07f9f3f6be` and tested local commit `3639dd3257683c1af43632ccdc47c28648df6f2b` have the identical complete tree `32ab55a98d801320f0bc92e44f54c32da6f90c66`; all 12 changed file hashes were verified against the public GitHub API at 12:38:37 UTC. The PR contains the gallery correction, six component regressions and ten documentation updates. Both GitHub checks passed. No application code changed after the validation above.

Vercel [EAcKaqygZvHJx9RWuL3gYteccK26](https://vercel.com/nugee/scuri/EAcKaqygZvHJx9RWuL3gYteccK26) is **Ready / Latest / Preview**, source `f777149`, with a 31-second build ending at 13:38:57 BST. Preview: https://scuri-git-fix-project-photos-scroll-nugee.vercel.app (immutable https://scuri-fd860doux-nugee.vercel.app). This PR has not been merged and production remains the PR #28 release. The four Drive documents were updated live, while repository updates are on the review branch. Physical iPad Safari momentum and live cloud round trips remain untested. Publication evidence: `scroll-release-branch-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)) and `SCURI_SCROLL_AND_DOCUMENTATION_2026-09-20.md`. This publication note is local only; the branch already contains the implementation and verification record above.

## Historical record — Project photos scrolling production — 20 September 2026

Following the user's explicit deployment request, [PR #29](https://github.com/NewGhee98/scuri/pull/29), "Fix Project photos scrolling and refresh Scuri documentation", merged into `main` as `24075d1e4c8cd6dfc2936d77332f8cd3793bf268`. Both GitHub checks passed and there were no merge conflicts. Published head `f777149`, merged main and tested local commit `3639dd3` share the identical complete tree `32ab55a98d801320f0bc92e44f54c32da6f90c66`; all 12 changed file hashes match. Fresh public verification at 12:42:55 UTC confirmed the merged tree. No application code changed after the 512-test/typecheck/lint/build and desktop scrolling checks above.

Vercel production [G6GijpLztNtqiQNT966cjQms2S7Z](https://vercel.com/nugee/scuri/G6GijpLztNtqiQNT966cjQms2S7Z) was verified **Ready / Latest / Production / Current**, source `24075d1`, with `scuri.vercel.app` assigned. The 42-second build completed at 13:43:21 BST (12:43:21 UTC). Immutable deployment: https://scuri-2f1pl3twe-nugee.vercel.app. The public site returned HTTP 200 at 12:44:05 UTC; all 11 JavaScript/CSS assets loaded and all 55 retained feature checks passed. These checks used no account session or project data; source identity establishes inclusion of the tested scrolling fix, while asset markers are only a deployment smoke check.

All four Drive documents now start with a verified production update explicitly superseding their earlier PR #28/awaiting-release status. Export read-back confirms the new main commit and deployment link, with every previous nonempty line retained. The ten repository documentation updates are now merged in main; this subsequent release record is local only. Physical iPad Safari momentum, Pencil/pinch and live cloud round trips remain untested. Refresh Scuri normally on each editing device to load the fix; do not clear browser storage. No saved composition, crop, photo original, Supabase/Drive asset, schema, migration, credential or live recovery change occurred.

Evidence: `scroll-release-branch-verification.json` (legacy artifact; see [evidence index](../evidence/README.md#legacy-local-artifacts)), `scroll-production-main-verification.json`, `scroll-production-public-verification.json`, `scuri-docs-2026-09-20/production-readback.json` and `SCURI_SCROLL_PRODUCTION_2026-09-20.md`.

## Historical record — Guidance for future coding agents / chats

Earlier deployment/version statements are historical; the latest production release is recorded in Project photos scrolling production above (PR #29, main `24075d1`). Recheck live state before further changes.
