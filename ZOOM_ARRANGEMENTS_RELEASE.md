...
Historical 13 September implementation record, now included in PR #29 production. PR #27 superseded negative-zoom centring: photos move freely on both axes, preserving legacy crops until edited. Current limits are 250 unique library photos and 30 pages. [START_HERE](START_HERE.md), [PROJECT_CONTEXT](PROJECT_CONTEXT.md) and [VERIFICATION](VERIFICATION.md) own current status, behaviour and checks. Below are original package decisions, not instructions to apply patches or repeat the migration; see the [evidence index](docs/evidence/README.md#legacy-local-artifacts).
