...
export async function createProjectBackup(
  ...
  let size = 0;
  const unique = new Map(snapshot.pages.flatMap(page => Object.values(page.photos).map(photo => [photo.blobKey, photo] as const)));
...
export async function inspectProjectBackup(blob: Blob): Promise<ProjectBackupPreview> {
  ...
  const expectedKeys = new Set(manifest.project.pages.flatMap(page => Object.values(page.photos).map(photo => photo.blobKey)));
...
export function materializeProjectBackup(preview: ProjectBackupPreview, generateId = () => crypto.randomUUID(), timestamp = new Date().toISOString()): { project: StoredProject; originals: Map<string, Blob> } {
  ...
  const keys = new Map(preview.project.pages.flatMap(page => Object.values(page.photos).map(photo => photo.blobKey)).map(key => [key, ""]));
