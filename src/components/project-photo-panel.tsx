...
export function ProjectPhotoPanel({ project, templates, ownerId, accessRevision, busy, getVolatileBlob, getDriveToken, onImport, onApply, onChoose,
  ...
  return <section className="photo-library-panel" aria-label="Project photo library">
    ...
    <dialog ref={dialog} data-open={String(open)} className="photo-library-dialog" onCancel={event => { event.preventDefault(); onClose(); }} aria-labelledby="photo-library-title">
      <div className="photo-library-shell">
        <header className="library-heading"><div><h2 id="photo-library-title">Project photos</h2><p>{project.name}{targetLabel ? ` · ${targetLabel}` : ""}</p></div>
          <button className="secondary-button" type="button" onClick={onClose}>{!photos.length ? "Skip for now" : "Done"}</button></header>
        {open ? <><div className="library-toolbar">
          <PhotoImportMenu onImport={onImport} initiallyOpen={initiallyImport} externalPicker={() => {
          ...
          <label>Sort <select value={view.sort} onChange={event => changeView({ ...view, sort: event.target.value as LibraryView["sort"], scrollTop: 0, anchor: undefined })}>
            <option value="import">Import order</option><option value="filename">Filename</option></select></label>
          <label>Thumbnails <select value={view.size} onChange={event => changeView({ ...view, size: event.target.value as LibraryView["size"] })}>
            <option value="small">Small</option><option value="medium">Medium</option><option value="large">Large</option></select></label>
          <button className="small-button" type="button" disabled={busy || photos.length < 2} onClick={() => setReviewDuplicates(true)}>Find duplicates</button>
          <button className="small-button" type="button" disabled={busy || suggesting || !Object.keys(analysed).length} onClick={() => void suggest()}>Suggest arrangements</button>
        ...
        {!inspectedRow ? <div className="library-filters">
          <div aria-label="Photo proportions">{(Object.keys(labels) as Orientation[]).map(key => <button key={key} type="button" className="library-filter" aria-pressed={view.orientations.includes(key)} onClick={() => toggleOrientation(key)}>{labels[key]}</button>)}</div>
          <div aria-label="Photo colour">{([["bw", "Black & white"], ["colour", "Colour"], ["uncertain", "Uncertain"], ["awaiting", "Awaiting analysis"]] as const).map(([key, label]) => <button key={key} type="button" className="library-filter" aria-pressed={view.colours.includes(key)} onClick={() => toggleColour(key)}>{label}</button>)}
            <label>Usage <select value={view.usage} onChange={event => changeView({ ...view, usage: event.target.value as LibraryView["usage"], scrollTop: 0, anchor: undefined })}>
              <option value="all">All</option><option value="unused">Unused</option><option value="used">Used</option></select></label>
            <span>{filtered.length} / {rows.length} photos</span><button className="text-button" type="button" onClick={() => changeView({ ...DEFAULT_LIBRARY_VIEW, size: view.size, sort: view.sort })}>Clear filters</button></div>
        </div> : null}
        <div className="library-progress">
          ...
          <span>Originals backed up: {photos.filter(photo => photo.driveOriginalId).length}/{photos.length} · Previews: {photos.filter(photo => photo.drivePreviewId).length}/{photos.length}</span>
          <details><summary>Backup details</summary><p>Project metadata saves independently. Originals without a Drive reference are only on this device until backup completes. Keep Scuri open; iPadOS may suspend background work.</p>
            ...
            <button type="button" className="text-button" onClick={onRetryBackup}>Retry backup / reconnect Drive</button></details>
    ...
    {proposals.length ? <div className="mt-6 grid gap-4 lg:grid-cols-3">{proposals.map(proposal => <article key={proposal.mode} className="rounded-xl border border-black/15 bg-white p-4">
      ...
      <button type="button" className="primary-button mt-4 w-full" disabled={busy || suggesting} onClick={() => onApply(proposal)}>Apply as new project</button>
            ...
            onBack={() => { setReturnFocusKey(inspectedRow.photo.blobKey); setInspected(null); }} onPrevious={() => { if (inspectIndex > 0) setInspected(filtered[inspectIndex - 1].photo.blobKey); }}
          ...
          !photos.length ? <div className="library-empty"><h3>Add your candidate photos once</h3><p>Choose Photos, Files or a connected source above. You can choose layouts later.</p></div> :
          <PhotoLibraryGallery rows={filtered} view={view} onView={value => { setReturnFocusKey(undefined); changeView(value); }} focusPhotoKey={returnFocusKey}
            onInspect={key => { setInspected(key); setShowSuggestions(false); }} />}
        <p className="library-footnote">Original files stay in your library. Exact duplicate checks use original bytes; older unverified files may need Find duplicates. Portable ZIP backups are limited to 256 MB.</p>
